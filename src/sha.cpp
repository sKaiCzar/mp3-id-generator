...
void SHA1::hash32() {
    ...
    for (int i = 0; i < chunks.size(); i++) {
        for (int j = 16; j < 80; j++) {
            chunks[i][j] = fmod(SHA1::rotateLeft(chunks[i][j-3] ^ chunks[i][j-8] ^ chunks[i][j-14] ^ chunks[i][j-16], 1), pow(2, 32));
        }
    }
    ...
    for (int i = 0; i < chunks.size(); i++) {
        ...
        for (int j = 0; j < 80; j++) {
            ...
            uint32_t temp = fmod(SHA1::rotateLeft(a, 5) + f + e + k + chunks[i][j], pow(2, 32));
            e = d;
            d = c;
